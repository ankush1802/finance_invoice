var ActiverInvoiceReceivedInvoiceListGrid;
$(document).ready(function () {
    ActiverInvoiceReceivedInvoiceListGrid = $("#ActiverInvoiceReceivedInvoiceListGrid").grid({
        dataKey: "Id",
        uiLibrary: "bootstrap",
        columns: [
...
function editPdf(e) {
    ...
    var col_index = 7;
    var IsCustomer = $('#ActiverInvoiceReceivedInvoiceListGrid tr:eq(' + row_index + ') td:eq(' + col_index + ')').find("div").attr('title');
    var Id = e.data.id;
    $.ajax({
...
function SendMailButton() {
    $(document).ready(function () {
        $('#ActiverInvoiceReceivedInvoiceListGrid tr').each(function () {
...
function editEmployee(e) {
    var url = "/Kippin/Finance/Invoice/EditInvoiceReceivedInvoice?id=REPLACEME";
