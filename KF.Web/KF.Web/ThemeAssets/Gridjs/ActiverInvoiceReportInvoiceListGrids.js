var ActiverInvoiceReportInvoiceListGrid;
$(document).ready(function () {
    ActiverInvoiceReportInvoiceListGrid = $("#ActiverInvoiceReportInvoiceListGrid").grid({
        dataKey: "Id",
        uiLibrary: "bootstrap",
...
function SendMailButton() {
    $("#ActiverInvoiceReportInvoiceListGrid tr").each(function () {
        ...
        if (status == "Pending Payment" || status == "Partial Payment") {
            ...
            if (status_cancel == "true") {
                ...
                $(this).find("td").eq(24).find("div").html("<button style='color:green;display:block;' class='btn btn-default clsCancel' type='button'>Cancel</button>");
            }
        }
...
$(document).on("click", ".clsModalManual", function (e) {
    ...
    var InvoiceId = $(this).closest('tr').find('td:eq(18)').text();
...
$(document).on("click", ".clsSubmitManualPayment", function (e) {
    if ($("#txtAmountPaid").val() == '') {
        ...
        $('#ActiverInvoiceReportInvoiceListGrid tr:eq(' + rowVal + ') td:eq(' + 15 + ')').find("div").find("label").text(amount);
        $("#txtAmountPaid").val('');
        $('#ActiverInvoiceReportInvoiceListGrid tr:eq(' + rowVal + ') td:eq(' + 17 + ')').find("div").find("button").removeAttr('disabled');
        $('#ActiverInvoiceReportInvoiceListGrid tr:eq(' + rowVal + ') td:eq(' + 21 + ')').find("div").find("button").removeAttr('disabled');
...
$(document).on("click", ".clsFinalManualPayments", function (e) {
    ...
    rowVal = rowVal + 1;
    var outStandingBal = $('#ActiverInvoiceReportInvoiceListGrid tr:eq(' + rowVal + ') td:eq(' + 9 + ')').find("div").attr('title');
    var amountPaid = $('#ActiverInvoiceReportInvoiceListGrid tr:eq(' + rowVal + ') td:eq(' + 15 + ')').find("div").find("label").text();
    var jvid = $('#ActiverInvoiceReportInvoiceListGrid tr:eq(' + rowVal + ') td:eq(' + 16 + ')').find("div").find("label").text();
    var IsCustomer = $('#ActiverInvoiceReportInvoiceListGrid tr:eq(' + rowVal + ') td:eq(' + 20 + ')').find("div").attr('title');
    var invoiceId = $('#ActiverInvoiceReportInvoiceListGrid tr:eq(' + rowVal + ') td:eq(' + 19 + ')').find("div").attr('title');
    var supplierid = $('#ActiverInvoiceReportInvoiceListGrid tr:eq(' + rowVal + ') td:eq(' + 23 + ')').find("div").attr('title');
    ...
    if ((parseFloat(amountPaid) > parseFloat(outStandingBal)) || amountPaid == 0) {
        ...
        $.ajax({
            type: "POST",
            url: '/Kippin/Finance/Invoice/InvoiceManualReportInvoice',
            data: dataToSend,
            dataType: 'json',
            contentType: false,
            processData: false,
...
$(document).on("click", ".clsSubmitJVIDPayment", function (e) {
    if ($("#txtJVIDPaid").val() == '') {
        ...
        $('#ActiverInvoiceReportInvoiceListGrid tr:eq(' + rowVal + ') td:eq(' + 16 + ')').find("div").find("label").text(amount);
        $("#txtJVIDPaid").val('');
...
$(document).on("click", ".clsSubmitStripPayment", function () {
    ...
    if ($("#txtAmountStripPaid").val() == '' && $("#txtStripJvid").val() == '' && $("#txtCardNumber").val() == '' && $("#txtCvv").val() == '') {
        ...
        if ((parseFloat($("#txtAmountStripPaid").val()) >= parseFloat(outStandingBal)) && $("#txtAmountStripPaid").val() == 0) {
            ...
            $.ajax({
                type: "POST",
                url: '/Kippin/Finance/Invoice/InvoiceStripReportInvoice',
                data: dataToSend,
                dataType: 'json',
                contentType: false,
                processData: false,
                ...
                //},
                //error: function (error) {
                //    //location.href = "@Url.Action('InvoiceReportInvoice', 'Invoice')";
                //    location.href = "/Invoice/InvoiceReportInvoice";
                //}
            });
        }
